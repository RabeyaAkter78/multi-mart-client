...
const CountdownCard = ({ endTime }) => {
  ...
  return <p className="text-lg font-bold text-red-500">{timeLeft}</p>;
};
  ...
  return (
    <div className="p-2 w-full">
      {products.map((product) => (
        <div
          key={product.id}
          className="bg-card-color text-black text-center w-full flex flex-col mb-5"
        >
          <Image
            src={product.image}
            width={0}
          ...
          <div className=" px-2">
            ...
            <p>{product.price}</p>
            <button className="bg-button-color px-4 py-2 rounded-md text-white mb-5">
              Shop Now
            </button>
          </div>
        </div>
